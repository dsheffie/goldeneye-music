...
int main(int argc, char *argv[]) {
  ...
  guest_t g(rom);
  memcpy(g.ptr(G_CTL), ctl, GE_ROM_INST_CTL_LEN);
  ...
  for(uint32_t f = 0; f < n_frames; f += 10) {
    ...
    uint32_t src = ((G_TBL & 0x1fffffffu) + base + 9*f);
    uint32_t skew = src & 7, nbytes = (9*nf + skew + 7) & ~7u;
    std::vector<uint32_t> cl = {
      0x07000000u, 0,                                        /* aSegment(0,0)            */
      0x0b000000u | (16*order*npred), (G_CTL & 0x1fffffffu) + book + 8,  /* aLoadADPCM  */
    ...
    const uint32_t task[16] = {2, 0, GE_rspbootText & 0x1fffffffu, GE_rspbootText_LEN,
			       ...
			       0, 0, 0, 0, G_CMDLIST & 0x1fffffffu, static_cast<uint32_t>(cl.size()*4), 0, 0};
    for(int i = 0; i < 16; i++) {
